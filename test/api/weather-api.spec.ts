...
describe("WeatherApi", () => {
  ...
  describe("token retrieval", () => {
    ...
    it("throws AuthenticationError when username missing", async () => {
      ...
      await expect(api.retrieveToken()).rejects.toBeInstanceOf(
        AuthenticationError,
      );
    });
  ...
  describe("device and state", () => {
    ...
    it("retrieveState updates state when API returns status 0", async () => {
      ...
      expect(api.getRealtimeState()).toEqual({
        deviceMac: "AA",
        sensorDatas: [],
      });
    });
    ...
    it("error branches: non-zero status responses", async () => {
      ...
      mockedAxios.mockResolvedValueOnce({
        data: { status: 1, errorMsg: "bad" },
      } as any);
      const api1 = new WeatherApi(username, password);
      await expect(api1.retrieveToken()).rejects.toBeInstanceOf(
        AuthenticationError,
      );

      // getBoundDevice non-zero
      mockedAxios.mockResolvedValueOnce({
        data: { status: 0, content: { token: "t" } },
      } as any);
      ...
      await expect(api2.getBoundDevice()).rejects.toThrow(
      ...
      await expect(api3.retrieveState()).rejects.toThrow(
        "No valid response from API",
      );
    });
  });
